 ...
 *
 */
	private const string CONFIG_MMF_NAME = "{8BA1E16C-FC54-4595-9782-E370A5FBE8DA}";

	private static readonly JsonSerializerOptions s_jsonSerializerOptions = new(JsonSerializerDefaults.General)
	{
		PropertyNameCaseInsensitive = true
	};

	public static NvServerConfig GetConfig()
	{

		using var cfg = MemoryMappedFile.OpenExisting(CONFIG_MMF_NAME);
		using var str = cfg.CreateViewStream();
		using var sr  = new StreamReader(str);

		var cfgData = sr.ReadToEnd().Trim('\0');

		return JsonSerializer.Deserialize<NvServerConfig>(cfgData, s_jsonSerializerOptions);
	}

	public FlurlClient Client { get; }

	public NvServerConfig Config { get; }

	public Nv(NvServerConfig config)
	{
		Config = config;
